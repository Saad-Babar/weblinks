...
// Set Follow/Nofollow attribute
$rel = (!empty($this->item->follow) && $this->item->follow === 'nofollow') ? 'nofollow' : 'follow';
?>
...
<div class="item-page">
    ...
    <div itemprop="articleBody">
        <div class="p-3">
            <a href="<?php echo $weblinkUrl; ?>" target="_blank" itemprop="url" rel="<?php echo $rel; ?>">
                <?php echo $weblinkUrl; ?>
            </a>
        </div>
        ...
        <div class="p-3">
            ...
            <?php if (!empty($images->image_first)) : ?>
                <?php $imgFloat = !empty($images->float_first) ? ($images->float_first == 'right' ? 'float-end' : 'float-start') : ''; ?>
                <?php $img = HTMLHelper::cleanImageURL($images->image_first); ?>
                <?php $alt = empty($images->image_first_alt) && empty($images->image_first_alt_empty) ? '' : 'alt="' . htmlspecialchars($images->image_first_alt, ENT_COMPAT, 'UTF-8') . '"'; ?>
                <figure class="item-image <?php echo $imgFloat; ?>">
                    <img src="<?php echo htmlspecialchars($img->url, ENT_COMPAT, 'UTF-8'); ?>" <?php echo $alt; ?> itemprop="thumbnail" />
            ...
            <?php if (!empty($images->image_second)) : ?>
                <?php $imgFloat = !empty($images->float_second) ? ($images->float_second == 'right' ? 'float-end' : 'float-start') : ''; ?>
                <?php $img = HTMLHelper::cleanImageURL($images->image_second); ?>
                <?php $alt = empty($images->image_second_alt) && empty($images->image_second_alt_empty) ? '' : 'alt="' . htmlspecialchars($images->image_second_alt, ENT_COMPAT, 'UTF-8') . '"'; ?>
                <figure class="item-image <?php echo $imgFloat; ?>">
                    <img src="<?php echo htmlspecialchars($img->url, ENT_COMPAT, 'UTF-8'); ?>" <?php echo $alt; ?> itemprop="thumbnail" />
